...
| --- | --- |
| A – Core functions | 19 of 20 requirements implemented. A15 (product ID substring search) is not implemented; the vendor catalogue supports exact-match product ID lookup only. |
...
Block A defines the storefront functions used by customers and the administration portal used by vendors. Of the twenty requirements, nineteen are fully implemented. Only A15 (product ID substring search) is not implemented; the vendor catalogue supports exact-match lookup of a product ID but not substring search.
...
| A15 | A system-generated product ID is shown. The vendor can search by entering a substring of the product ID. | Not implemented | The catalogue displays the product ID and supports exact-match ID lookup, but does not support substring search of the product ID as specified. |
